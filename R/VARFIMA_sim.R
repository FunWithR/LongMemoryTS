...
#d.vec<-c(0.4,0.3)
#THETA<-matrix(c(theta11,theta21,theta12,theta22),2,2)

#theta<-c(d.vec,phi,theta11,theta21,theta12,theta22,Sigma[1,1],Sigma[2,2],Sigma[1,2])

#data<-varfima.series<-VARFIMA.sim(phi=phi,THETA=THETA,d.vec=d.vec,T=T, Sigma=Sigma)
#ts.plot(varfima.series, col=1:2)

#data<-varfima.series

#fdGPH(varfima.series[,1])
#fdGPH(varfima.series[,2])

#arima(varfima.series[,1], order=c(1,0,1))
#arima(varfima.series[,2], order=c(1,0,1))


#acf(varfima.series)


#varfima.series<-VARFIMA.sim(phi=0.4,M1=M1,d.vec=c(0.4,0.2),T=1000)

#summary(fracdiff(varfima.series[,1], nar=1, nma=1))
#summary(fracdiff(varfima.series[,2], nar=1, nma=1))

#test<-fracdiff.sim(1000, ar=0.6, ma=0.8, d=0)$series
#ts.plot(test)
#acf(test)
#summary(fracdiff(test, nar=1, nma=1))
